...
for k in S3_ACCESS_KEY S3_SECRET_KEY; do
  V=$($K get secret "${RELEASE}-app-secrets" -o jsonpath="{.data.$k}" 2>/dev/null | base64 -d 2>/dev/null || true)
  [ -n "$V" ] && ok "${RELEASE}-app-secrets/$k renseigné" \
    || ko "${RELEASE}-app-secrets/$k vide : elearning-api ne démarre pas (S3_ACCESS_KEY=… S3_SECRET_KEY=… scripts/seal-secrets.sh)"
done
